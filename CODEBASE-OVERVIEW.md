...
| `App\Http\Controllers\Api\ReportController` | `GET /api/reports` (`?archived=1` per i soli archiviati), `GET /api/reports/{id}`, `GET /api/reports/generators`, `POST /api/reports/ingest`, `POST /api/reports/{id}/archive`, `POST /api/reports/{id}/unarchive`, `DELETE /api/reports/{id}` | Lista report paginata; dettaglio con news items; ingest via API (form UI); archiviazione reversibile (`archived_at`, esclusa di default dall'index); eliminazione con riconciliazione cluster |
...
| `App\Http\Controllers\Api\PublicationController` | `GET /api/publications` (`?archived=1` per le sole archiviate), `PATCH /api/publications/{id}`, `GET /api/publications/{id}/export`, `POST /api/publications/{id}/archive`, `POST /api/publications/{id}/unarchive` | Lista pubblicazioni; aggiornamento stato/contenuto; download markdown; archiviazione reversibile indipendente dallo stato (`archived_at`), esclusa di default dall'index |
